...
describe('Testes realizados no componente App', () => {
  ...
  test(
    'se ao digitar um email correto e uma senha menor o botão permanece desabilitado',
    () => {
      renderWithRouterAndRedux(<App />);
      const emailInput = screen.getByTestId(dataTestIdEmail);
      const passwordInput = screen.getByTestId(dataTestIdPassword);
  ...
  test(
    'ao digitar uma senha e um email incorreto, o botão permanece desabilitado',
    () => {
      renderWithRouterAndRedux(<App />);
      const emailInput = screen.getByTestId(dataTestIdEmail);
      const passwordInput = screen.getByTestId(dataTestIdPassword);
  ...
  test(
    'testa se ao digitar nos inputs recebem valores digitados e o Login é habilitado',
    () => {
      renderWithRouterAndRedux(<App />);
      const emailInput = screen.getByTestId(dataTestIdEmail);
      const passwordInput = screen.getByTestId(dataTestIdPassword);
  ...
  test(
    'testa se ao clicar no botão o email é salvo no estado global da aplicaçãp',
    () => {
